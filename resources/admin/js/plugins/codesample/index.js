...
const insertCodeSample = (editor, language, code, title) => {
    ...
    editor.undoManager.transact(() => {
        ...
        return node.fold(() => {
            ...
            if (title) {
                dom.setAttrib(newPre, 'data-language', title);
            }
            editor.selection.select(newPre);
        }, n => {
            dom.setAttrib(n, 'class', 'language-' + language);
            if (title) {
                dom.setAttrib(n, 'data-language', title);
            } else {
                n.removeAttribute('data-language');
            }
            n.innerHTML = code;
...
const getCurrentTitle = (editor) => {
    const node = getSelectedCodeSample(editor);
    return node.fold(() => '', n => n.dataset.language ?? '');
};
...
const open = editor => {
    ...
    const currentTitle = getCurrentTitle(editor);
    editor.windowManager.open({
        ...
        onSubmit   : api => {
            ...
            insertCodeSample(editor, data.language, data.code, data.title);
            api.close();
        },
    });
};
    ...
    editor.on('PreInit', () => {
        editor.parser.addNodeFilter('pre', nodes => {
            ...
            for (let i = 0, l = nodes.length; i < l; i++) {
                const node = nodes[i];
                const isCodeSample = ((_a = node.attr('class')) !== null && _a !== void 0
                                      ? _a
                                      : '').indexOf('language-') !== -1;
                if (isCodeSample) {
